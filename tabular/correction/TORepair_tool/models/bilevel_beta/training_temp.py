...
class BiLevelTrainer:
    ...
    def __init__(self, args, 
        ...
        self.noise_t_batch_level = getattr(self.args, 'noise_t_batch_level', False)
    ...
    def impute_with_model(self, df, dataset_name=None, error_mask_df=None):
        ...
        # 转换为可微分版本调用
        return self._impute_dataframe_with_differentiable_model(df, dataset_name, error_mask_df, with_grad=False)

    def _impute_dataframe_with_differentiable_model(self, df, dataset_name=None, error_mask_df=None, with_grad=False):
        """
    ...
    def compute_explicit_bilevel_loss(self, classifier, x_imp_batch, y_imp_batch):
        ...
        2. 计算 G_term = d/d_theta ( (d L_in / dw)^T * v )
        
        这个函数直接返回一个可以反向传播以更新theta（这里是x_imp_batch）的标量损失。
        The loss is - (d L_in / dw)^T * v.detach()
        """
        ...
        # 4. 计算最终的标量损失，其梯度就是我们需要的超梯度的一部分
        # loss = - (dL_in/dw)^T * (H_in^-1 * dL_out/dw).detach()
        # .detach()确保梯度只通过 dL_in/dw 项流向 x_imp_batch -> theta
        explicit_bilevel_loss = -torch.dot(
    ...
    def _perform_diffusion_pretraining_step(self, val_x, val_error_mask):
        ...
        # 构建条件化输入
        e_conditional_input = (e_0 * e_mask + 
                              e_t * (1 - e_mask - ori_err_mask_dim) + 
                              ori_err_mask_dim * e_0)
    ...
    def _perform_original_space_diffusion(self, x_num, x_cat, num_mask, cat_mask, num_steps=100):
        ...
        for i in range(num_steps-1, -1, -1):
            ...
            model_input = torch.cat([e_noisy_input, e_mask], dim=1)
    ...
    def _perform_direct_imputation(self, x_num, x_cat, num_mask, cat_mask):
        ...
        if hasattr(self.repair_model, 'predict_noise') and self.repair_model.predict_noise:
            ...
            e_conditional = e0_filled * emb_mask + e_t * (1 - emb_mask)
            # e_conditional = e0_filled
            
            # 构建2通道输入
            model_input = torch.cat([e_conditional, emb_mask], dim=1)
    ...
    def _perform_outer_step(self, val_error_x, val_error_y, val_error_mask, val_observed_mask,
        ...
        # 构建条件化输入：
        # e_0 * e_mask + e_t * (1 - e_mask - ori_err_mask_dim) + ori_err_mask_dim * e_0
        # e_conditional_input = (e_0_mixed * e_mask + 
        #                       e_t * (1 - e_mask - ori_err_mask_dim) + 
        #                       ori_err_mask_dim * e_0_mixed)
        e_conditional_input = e_0_mixed * e_mask + e_t * (1 - e_mask)
