...
# from transformers import Reconstructor, Tokenizer, Transformer
import torch
...
#     """
#         Input:
#             x_num: [bs, d_numerical*k]
#             x_cat: [bs, len(categories)*k]
#             neighbor_features: [bs, embedding_dim] (optional)
#         Output:
...
#             dim_t=512, 
#             use_mlp=True, 
#             use_neighbor_conditioning=True, **kwargs
#         ):
#         super().__init__()
#         self.d_numerical = d_numerical
#         self.categories = categories
#         self.use_neigh_cond = use_neighbor_conditioning
#         self.neigh_num = k
#         self.d_token = d_token

#         self.tokenizer = Tokenizer(d_numerical, categories, d_token, bias = bias)
#         self.encoder = Transformer(num_layers, d_token, n_head, d_token, factor)
#         d_in = d_token * (k+2) *(d_numerical + len(categories))
#         self.mlp = MLPDiffusion(d_in, dim_t=d_in, use_mlp=use_mlp)
...
#     def forward(self, x_num: torch.Tensor, x_cat: torch.Tensor, timesteps: torch.Tensor, neighbor_features: dict):
#         e = self.tokenizer(x_num, x_cat)
#         decoder_input_ori = e[:, 1:, :]
#         batch_size = x_num.shape[0]

#         if self.use_neigh_cond:
#             num_dim = x_num.shape[-1]
#             cat_dim = x_cat.shape[-1]
#             cond_num = neighbor_features['num'].view(-1, num_dim)
#             cond_cat = neighbor_features['cat'].view(-1, cat_dim)

#             cond_features = self.tokenizer(cond_num, cond_cat)
#             cond_features = cond_features[:, 1:, :]
#             decoder_input = torch.cat([decoder_input_ori, cond_features.reshape(batch_size, -1, self.d_token)], dim=1)
#         y = self.encoder(decoder_input)
#         y = torch.cat([y, decoder_input_ori], dim=1)
...
#     def get_conditioning_features(self, x_num, x_cat, timesteps):
#         """Extract features for conditioning from intermediate layers"""
#         # XXX: Whether need this to encode?
#         e = self.tokenizer(x_num, x_cat)
#         decoder_input = e[:, 1:, :]
#         y = self.encoder(decoder_input)
...
#     def forward(self, x_num, x_cat, t, sigma, neighbor_features=None):
...
#             F_x, x_cat_pred = self.denoise_fn_F(x_in, x_cat, c_noise.flatten(), neighbor_features)
#         elif self.net_conditioning == "t":
#             F_x, x_cat_pred = self.denoise_fn_F(x_in, x_cat, t, neighbor_features)
...
class MLPClassifier(nn.Module):
    ...
    def _initialize_weights(self):
        """
        Initialize the weights of the model using He initialization for ReLU activations.
        """
